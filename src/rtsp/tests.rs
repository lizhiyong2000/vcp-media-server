...
async fn test_rtsp_announce_and_record_scenario() {
    ...
    let _ = RtspServer::process_rtsp_request(&announce_request, &manager, &mut session, peer_addr, None, None).await;
    sleep(Duration::from_millis(10)).await;
    
    let setup_request = "SETUP rtsp://localhost:554/record_stream/trackID=0 RTSP/1.0\r\nCSeq: 3\r\nTransport: RTP/AVP/TCP;interleaved=0-1\r\n\r\n";
    info!("Sending SETUP request...");
    let _ = RtspServer::process_rtsp_request(setup_request, &manager, &mut session, peer_addr, None, None).await;
    sleep(Duration::from_millis(10)).await;
    ...
    let _ = RtspServer::process_rtsp_request(record_request, &manager, &mut session, peer_addr, None, None).await;
