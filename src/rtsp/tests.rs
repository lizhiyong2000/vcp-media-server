...
async fn test_rtsp_logging_scenario() {
    ...
    let _ = RtspServer::process_rtsp_request(options_request, &manager, &mut session, peer_addr).await;
    sleep(Duration::from_millis(10)).await;
    
    let describe_request = "DESCRIBE rtsp://localhost:554/test_stream RTSP/1.0\r\nCSeq: 2\r\nAccept: application/sdp\r\n\r\n";
    info!("Sending DESCRIBE request...");
    let _ = RtspServer::process_rtsp_request(describe_request, &manager, &mut session, peer_addr).await;
    sleep(Duration::from_millis(10)).await;
    ...
    let _ = RtspServer::process_rtsp_request(setup_video_request, &manager, &mut session, peer_addr).await;
    sleep(Duration::from_millis(10)).await;
    ...
    let _ = RtspServer::process_rtsp_request(setup_audio_request, &manager, &mut session, peer_addr).await;
    sleep(Duration::from_millis(10)).await;
    
    let play_request = "PLAY rtsp://localhost:554/test_stream RTSP/1.0\r\nCSeq: 5\r\nSession: test-session\r\nRange: npt=0.000-\r\n\r\n";
    info!("Sending PLAY request...");
    let _ = RtspServer::process_rtsp_request(play_request, &manager, &mut session, peer_addr).await;
    sleep(Duration::from_millis(10)).await;
    
    let pause_request = "PAUSE rtsp://localhost:554/test_stream RTSP/1.0\r\nCSeq: 6\r\nSession: test-session\r\n\r\n";
    info!("Sending PAUSE request...");
    let _ = RtspServer::process_rtsp_request(pause_request, &manager, &mut session, peer_addr).await;
    sleep(Duration::from_millis(10)).await;
    
    let play_request_2 = "PLAY rtsp://localhost:554/test_stream RTSP/1.0\r\nCSeq: 7\r\nSession: test-session\r\n\r\n";
    info!("Sending PLAY request again...");
    let _ = RtspServer::process_rtsp_request(play_request_2, &manager, &mut session, peer_addr).await;
    sleep(Duration::from_millis(10)).await;
    
    let teardown_request = "TEARDOWN rtsp://localhost:554/test_stream RTSP/1.0\r\nCSeq: 8\r\nSession: test-session\r\n\r\n";
    info!("Sending TEARDOWN request...");
    let _ = RtspServer::process_rtsp_request(teardown_request, &manager, &mut session, peer_addr).await;
...
async fn test_rtsp_announce_and_record_scenario() {
    ...
    let _ = RtspServer::process_rtsp_request(&announce_request, &manager, &mut session, peer_addr).await;
    sleep(Duration::from_millis(10)).await;
    
    let setup_request = "SETUP rtsp://localhost:554/record_stream/trackID=0 RTSP/1.0\r\nCSeq: 3\r\nTransport: RTP/AVP/TCP;interleaved=0-1\r\n\r\n";
    info!("Sending SETUP request...");
    let _ = RtspServer::process_rtsp_request(setup_request, &manager, &mut session, peer_addr).await;
    sleep(Duration::from_millis(10)).await;
    ...
    let _ = RtspServer::process_rtsp_request(record_request, &manager, &mut session, peer_addr).await;
...
async fn test_rtsp_error_scenario() {
    ...
    let _ = RtspServer::process_rtsp_request(invalid_request, &manager, &mut session, peer_addr).await;
    sleep(Duration::from_millis(10)).await;
    
    let empty_request = "";
    info!("Sending empty request...");
    let _ = RtspServer::process_rtsp_request(empty_request, &manager, &mut session, peer_addr).await;
    sleep(Duration::from_millis(10)).await;
    
    let bad_line_request = "INVALID LINE WITHOUT SPACES\r\nCSeq: 1\r\n\r\n";
    info!("Sending request with invalid first line...");
    let _ = RtspServer::process_rtsp_request(bad_line_request, &manager, &mut session, peer_addr).await;
    sleep(Duration::from_millis(10)).await;
    
    let describe_not_found = "DESCRIBE rtsp://localhost:554/nonexistent RTSP/1.0\r\nCSeq: 2\r\n\r\n";
    info!("Sending DESCRIBE for non-existent stream...");
    let _ = RtspServer::process_rtsp_request(describe_not_found, &manager, &mut session, peer_addr).await;
    sleep(Duration::from_millis(10)).await;
    
    let play_no_stream = "PLAY rtsp://localhost:554/nonexistent RTSP/1.0\r\nCSeq: 3\r\n\r\n";
    info!("Sending PLAY without selecting stream...");
    let _ = RtspServer::process_rtsp_request(play_no_stream, &manager, &mut session, peer_addr).await;
