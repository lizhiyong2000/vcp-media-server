...
use crate::decoder::nal::sps::{ConstraintFlags, Level, ProfileIdc, SeqParameterSet};
use crate::decoder::nal::{pps, sps, Nal, NalHeader, NalHeaderError, RefNal, UnitType};
...
impl<'buf> AvcDecoderConfigurationRecord<'buf> {
    ...
    pub fn create_context(&self) -> Result<Context, AvccError> {
        ...
        for sps in self.sequence_parameter_sets() {
            ...
            let sps = crate::decoder::nal::sps::SeqParameterSet::from_bits(sps.rbsp_bits())
        ...
        for pps in self.picture_parameter_sets() {
            ...
            let pps = crate::decoder::nal::pps::PicParameterSet::from_bits(&ctx, pps.rbsp_bits())
...
mod test {
    use super::*;
    use crate::decoder::nal::pps::PicParamSetId;
    use crate::decoder::nal::sps::SeqParamSetId;
    use hex_literal::*;
