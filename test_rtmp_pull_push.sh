...
# RTMP 推流和播放测试脚本（ffmpeg循环推流版本）
# 测试流程：启动媒体服务器 -> 启动循环RTMP推流 -> 启动ffplay播放 -> 验证播放

# 配置
MEDIA_SERVER_BIN="./target/release/media-server"
TEST_VIDEO="./test_video.mp4"
RTMP_PUSH_URL="rtmp://localhost:1935/live"
RTMP_PLAY_URL="rtmp://localhost:1935/live"
TEST_DURATION=30  # 播放持续时间（秒）
LOG_FILE="/tmp/rtmp_test.log"

echo "=========================================="
echo "RTMP 推流和播放测试"
echo "=========================================="
echo "视频文件: $TEST_VIDEO"
echo "推流地址: $RTMP_PUSH_URL"
echo "播放地址: $RTMP_PLAY_URL"
...
# 启动循环RTMP推流（-stream_loop -1 表示无限循环播放源视频）
echo "[4/5] 启动循环RTMP推流..."
ffmpeg -re -stream_loop -1 -i $TEST_VIDEO -vcodec libx264 -preset ultrafast -tune zerolatency -acodec aac -f flv $RTMP_PUSH_URL > /tmp/ffmpeg_rtmp_push.log 2>&1 &
FFMPEG_PID=$!
...
if ! pgrep -x "ffmpeg" > /dev/null; then
    echo "ERROR: ffmpeg RTMP 推流启动失败"
    cat /tmp/ffmpeg_rtmp_push.log
...
timeout ${TEST_DURATION}s ffplay $RTMP_PLAY_URL > /tmp/ffplay_rtmp.log 2>&1
FFPLAY_EXIT_CODE=$?
set -e  # 重新启用 set -e
...
# 检查是否有错误
if grep -E "error|Error|ERROR" /tmp/ffplay_rtmp.log > /dev/null; then
    echo "WARNING: ffplay 日志中存在错误信息"
    cat /tmp/ffplay_rtmp.log
fi
